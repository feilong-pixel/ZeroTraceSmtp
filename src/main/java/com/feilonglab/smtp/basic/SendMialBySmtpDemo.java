...
public class SendMialBySmtpDemo {
	...
	public static void main(String[] args) {
		...
		// 执行多收件人邮件发送示例（支持 TO/CC/BCC）
		sendMultiRecipientMail();
	}
	...
	/**
	 * 演示单封邮件同时发送给多收件人（包含 TO、CC、BCC 抄送/密送群发功能）。
	 */
	private static void sendMultiRecipientMail() {
		logger.info("开始执行【多收件人邮件发送示例（支持 TO/CC/BCC）】...");
		try (SmtpClient client = new SmtpClient()) {
			...
			String content = "<h1>多收件人测试</h1><p>这是一封同时发送给多个收件人、抄送人和密送人的测试邮件。</p>";

			// 步骤二：发送多收件人邮件
			try {
				client.sendMail(toList, ccList, bccList, subject, content);
			} catch (Exception e) {
				logger.error("【发送错误】多收件人邮件发送失败！", e);
			}
